...
try {
  ...
  for (const method of ['prepareCredential', 'testCredential', 'saveCredential', 'discardCredentials', 'checkForUpdate', 'setAutoUpdate', 'applyUpdate']) assert.ok(exposed.methods.includes(method))
  assert.equal(exposed.methods.includes('registerCredential'), false)
  await page.getByRole('button', { name: 'Atualizar o Omni' }).click()
  await page.getByRole('heading', { name: 'Atualizar o Omni.' }).waitFor()
  await page.getByRole('button', { name: 'Conferir agora' }).click()
  await page.getByText('Tudo em dia', { exact: true }).waitFor()
  await page.getByRole('button', { name: 'Fechar atualização do Omni' }).click()
