...
export function UpdatePanel({ status, onClose, onCheck, onAuto, onApply }: {
  ...
  return <section className="update-settings" role="dialog" aria-modal="true" aria-label="Atualização do Omni">
    ...
    <div className={'update-status ' + status.state} aria-live="polite"><i aria-hidden="true" /><div><strong>{label}</strong><p>{status.detail}</p><small>Build: {status.currentVersion}{status.availableVersion ? ` · nova: ${status.availableVersion}` : ''}{status.lastAppliedAt ? ` · aplicada: ${new Date(status.lastAppliedAt).toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'short' })}` : ''}{status.releaseVersion ? ` · núcleo: ${status.releaseVersion}` : ''}{status.releaseFingerprint ? ` · assinatura: ${status.releaseFingerprint}` : ''}{status.releaseIntegrity === 'verified' ? ' · íntegro' : status.releaseIntegrity === 'drifted' ? ' · divergente' : ''}</small></div></div>
