...
type ContinuityObservation = { fingerprint: string; recordedAt: string }
type DirectionObservation = ContinuityObservation & { directives: string[] }
type ContinuityState = { schemaVersion: 1; observations: ContinuityObservation[] }
type DirectionState = { schemaVersion: 1; observations: DirectionObservation[] }

function validFingerprint(value: unknown): value is string {
  return typeof value === 'string' && /^[a-f0-9]{64}$/u.test(value)
}

function validRecordedAt(value: unknown): value is string {
...
function parseContinuity(value: unknown): ContinuityState {
  ...
  if (item.schemaVersion !== 1 || Object.keys(item).length !== 2 || !Array.isArray(item.observations) || item.observations.length > 100) throw new Error('Unsupported continuity store.')
  const observations = item.observations.map((raw): ContinuityObservation => {
    if (raw === null || typeof raw !== 'object' || Array.isArray(raw)) throw new Error('Invalid continuity observation.')
    const row = raw as Record<string, unknown>
    if (Object.keys(row).length !== 2 || !validFingerprint(row.fingerprint) || !validRecordedAt(row.recordedAt)) throw new Error('Invalid continuity observation.')
    return { fingerprint: row.fingerprint, recordedAt: row.recordedAt }
  })
  return { schemaVersion: 1, observations }
}

function parseDirections(value: unknown): DirectionState {
  if (value === null) return { schemaVersion: 1, observations: [] }
  if (typeof value !== 'object' || Array.isArray(value)) throw new Error('Invalid personality direction store.')
  const item = value as Record<string, unknown>
  if (item.schemaVersion !== 1 || Object.keys(item).length !== 2 || !Array.isArray(item.observations) || item.observations.length > 100) throw new Error('Unsupported personality direction store.')
  const observations = item.observations.map((raw): DirectionObservation => {
    if (raw === null || typeof raw !== 'object' || Array.isArray(raw)) throw new Error('Invalid personality direction observation.')
    const row = raw as Record<string, unknown>
    if (Object.keys(row).length !== 3 || !validFingerprint(row.fingerprint) || !validRecordedAt(row.recordedAt) || !Array.isArray(row.directives) || row.directives.length === 0 || row.directives.length !== new Set(row.directives).size || !row.directives.every(validDirective)) throw new Error('Invalid personality direction observation.')
...
function directionsLocation(home: string): string {
  if (!isAbsolute(home)) throw new Error('Continuity home must be absolute.')
...
export async function observePersonalityContinuity(home: string, feedback: string, at?: string, directives: readonly string[] = []): Promise<void> {
  ...
  if (explicitPersistentDirective) {
    ...
    if (normalizedDirectives.length === 0) return
    await store.update(directionsLocation(home), current => {
      ...
      } else state.observations.push({ fingerprint, recordedAt, directives: normalizedDirectives })
      return { schemaVersion: 1, observations: state.observations.slice(-100) }
    })
  }
}
