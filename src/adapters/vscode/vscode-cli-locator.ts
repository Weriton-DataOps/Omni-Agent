...
import type {
  WorkspaceCliLaunchPlan,
  WorkspaceCliLocator
} from '../../ports/workspace-cli-locator.js'
import type { WorkspaceFileSystem } from '../../ports/workspace-filesystem.js'

const MAX_WINDOWS_WRAPPER_BYTES = 16 * 1_024

export class VscodeCliLocator implements WorkspaceCliLocator {
  constructor(
    private readonly fileSystem: Pick<WorkspaceFileSystem, 'exists' | 'readText'>,
  ...
  resolve(): WorkspaceCliLaunchPlan {
    ...
    const known = this.platform === 'win32'
      ? [
          this.environment.LOCALAPPDATA && api.join(
          ...
          this.environment.ProgramFiles && api.join(
    ...
    const found = candidates.find((candidate) =>
      api.isAbsolute(candidate) && this.fileSystem.exists(candidate)
    )
    if (!found) {
      throw new Error(
        `${executable} nao foi encontrado no PATH nem nas instalacoes conhecidas.`
      )
    }
  ...
  private resolveWindowsWrapper(wrapperPath: string): WorkspaceCliLaunchPlan {
    ...
    if (
      normalizedExecutable !== win32.join(installationRoot, 'Code.exe').toLowerCase() ||
