...
interface PlanBinding {
  ...
  strategyFingerprint: DocumentFingerprint
}
...
export interface AuthorizationDecision {
  ...
  requestBinding: RequestBinding
  planBinding: PlanBinding
  outcome: AuthorityEvaluation['outcome']
  actionDecisions: AuthorityEvaluation['actionDecisions']
  limits: AuthorityEvaluation['limits']
  revocation: {
...
function parseFingerprint(value: unknown, path: string): DocumentFingerprint {
  ...
  return {
    ...
    value: matchingString(input.value, `${path}.value`, SHA256) as `sha256:${string}`
  }
}

function parseRequestBinding(value: unknown): RequestBinding {
  ...
  return {
    ...
    requestFingerprint: parseFingerprint(
      ...
      'authorizationRequest.requestBinding.requestFingerprint'
    ),
    clientId: identifier(input.clientId, 'authorizationRequest.requestBinding.clientId')
  }
}

function parsePlanBinding(value: unknown): PlanBinding {
  ...
  return {
    ...
    strategyFingerprint: parseFingerprint(
      input.strategyFingerprint,
      'authorizationRequest.planBinding.strategyFingerprint'
    )
  }
}

function parseExternalAction(value: unknown, index: number): AuthorityAction {
  ...
  const scope = oneOf(input.scope, `${path}.scope`, ['request-resource', 'runtime-internal'] as const)
  const effectMode = oneOf(input.effectMode, `${path}.effectMode`, ['none', 'journaled'] as const)
  ...
  return {
    ...
    }) as AuthorityAction['requestedControls']
  }
}

function parseCeiling(value: unknown): unknown {
...
function assertRequestFingerprint(input: Record<string, unknown>, fingerprinter: DocumentFingerprinter): void {
  const declared = parseFingerprint(
    input.authorizationRequestFingerprint,
    'authorizationRequest.authorizationRequestFingerprint'
  )
  ...
  if (canonicalJson(declared) !== canonicalJson(fingerprinter.fingerprint(basis))) {
    throw new ContractValidationError(
      'authorizationRequest.authorizationRequestFingerprint',
      'fingerprint nao corresponde ao conteúdo'
    )
  }
}

function parseAuthorizationRequest(value: unknown, fingerprinter: DocumentFingerprinter): ParsedAuthorizationRequest {
  ...
  const requestResourceActionCount = integer(
    risk.requestResourceActionCount,
    'authorizationRequest.riskSummary.requestResourceActionCount',
    { min: 0, max: 256 }
  )
  const journaledEffectCount = integer(
    risk.journaledEffectCount,
    'authorizationRequest.riskSummary.journaledEffectCount',
    { min: 0, max: 256 }
  )
  ...
  const authorizationRequestId = identifier(
  ...
  const envelope = parseAuthorityEnvelope({
    ...
    triggeredBoundaries: uniqueStrings(
      risk.triggeredBoundaries,
      'authorizationRequest.riskSummary.triggeredBoundaries',
      { max: 8, allowed: authorizationContractV1.boundaries }
    )
  })
  return {
...
export class OvercoreAuthorityAdapter {
  constructor(
    ...
    private readonly fingerprinter: DocumentFingerprinter
  ) {}

  evaluate(input: unknown): AuthorizationDecision {
    ...
    const base = {
      ...
      issuer: {
        providerId: 'omni-authority-provider' as const,
        providerKind: 'assistant' as const,
        authorityBasisRef: 'omni-owner-authority-v1' as const
      },
      audience: {
      ...
      revocation: {
        mode: 'check-before-journaled-effect' as const,
...
export function evaluateOvercoreAuthorizationRequest(
  input: unknown,
  options: { at?: Date } = {}
): AuthorizationDecision {
  const at = options.at ?? new Date()
  return new OvercoreAuthorityAdapter(
    new AuthorityService({ now: () => at }),
    new NodeDocumentFingerprinter()
  ).evaluate(input)
}
