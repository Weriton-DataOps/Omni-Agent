...
import type { WorkspaceCliLocator } from '../../ports/workspace-cli-locator.js'
...
export interface OpenWorkspaceDependencies {
  readonly environment: Readonly<Record<string, string | undefined>>
  readonly defaultHubRoot: string | null
  readonly fileSystem: WorkspaceFileSystem
  readonly process: WorkspaceProcess
  readonly cliLocator: WorkspaceCliLocator
  readonly pathPolicy: WorkspacePathPolicy
  readonly evidenceFingerprinter: WorkspaceEvidenceFingerprinter
}

function outputOf(result: WorkspaceProcessResult): string {
  return `${result.stdout ?? ''}\n${result.stderr ?? ''}`.trim()
}

export function openWorkspace(
  request: unknown,
  dependencies: OpenWorkspaceDependencies
): WorkspaceOpenResult {
  const parsed = parseOpenWorkspaceRequest(request)
  if (!parsed.ok) throw new WorkspaceInputValidationError(parsed.issues)
  ...
  const status = dependencies.process.run(
    executable,
    [...prefixArgs, '--status'],
    launch.environment
  )
  const statusOutput = outputOf(status)
  const verified = status.status === 0 && dependencies.pathPolicy.statusContainsExactPath(
    statusOutput,
