...
} from '../adapters/overcore/authorization.js'
...
async function bodyOf(request: IncomingMessage): Promise<unknown> {
  ...
  for await (const chunk of request) {
    ...
    size += buffer.length
    if (size > MAX_BODY_BYTES) throw new Error('Pedido excede o limite de 1 MiB.')
    chunks.push(buffer)
  }
...
export interface OvercoreAuthorityServerOptions {
  token: string
  at?: () => Date
}

export function createOvercoreAuthorityServer(options: OvercoreAuthorityServerOptions): Server {
...
export async function startOvercoreAuthorityCli(): Promise<void> {
  const token = process.env.OMNI_AUTHORITY_PROVIDER_TOKEN
  if (!token) throw new Error('OMNI_AUTHORITY_PROVIDER_TOKEN não foi definido.')
  const host = '127.0.0.1'
  const port = Number(process.env.OMNI_AUTHORITY_PROVIDER_PORT ?? '47832')
  if (!Number.isInteger(port) || port < 0 || port > 65_535) throw new Error('Porta local inválida.')
  ...
  if (!address || typeof address === 'string') throw new Error('Servidor não informou porta TCP.')
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve(process.argv[1])) {
  await startOvercoreAuthorityCli().catch((error: unknown) => {
