...
function parseGrant(value: unknown, path: string): AuthorityGrant {
...
function parseCeiling(value: unknown, path: string): AuthorityCeiling {
  ...
  const result: AuthorityCeiling = {
    mode: 'proceed-within-scope',
    grants: array(input.grants, `${path}.grants`, { max: 64 }).map((grant, index) =>
      parseGrant(grant, `${path}.grants[${index}]`)
    ),
    expansionBoundaries: uniqueStrings(input.expansionBoundaries ?? [], `${path}.expansionBoundaries`, {
      max: 8,
      allowed: AUTHORITY_BOUNDARIES
    }) as AuthorityCeiling['expansionBoundaries']
  }
  if (input.expiresAt !== undefined) result.expiresAt = dateTime(input.expiresAt, `${path}.expiresAt`)
  return result
}

function parseAction(value: unknown, path: string): AuthorityAction {
...
export function parseAuthorityEnvelope(value: unknown): AuthorityEnvelope {
  ...
  return {
    ...
    triggeredBoundaries: uniqueStrings(input.triggeredBoundaries, 'authorityEnvelope.triggeredBoundaries', {
      max: 8,
      allowed: AUTHORITY_BOUNDARIES
    }) as AuthorityEnvelope['triggeredBoundaries']
  }
}
