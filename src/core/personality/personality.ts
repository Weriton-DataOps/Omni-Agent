export interface PersonalityManifestIdentity {
  readonly id: string
}

export interface PersonalityVoiceExample {
  readonly situation: string
  readonly line: string
}

export interface PersonalitySnapshot {
  readonly manifest: PersonalityManifestIdentity | null
  readonly nucleus: string | null
  readonly textAdapter: string | null
  readonly continuityAnchor: string | null
  readonly learnedAdjustmentText: string | null
  readonly gallery: readonly PersonalityVoiceExample[]
}

export interface PersonalityFeedbackSummary {
  readonly adjustment?: {
    readonly directives?: readonly string[]
  } | null
  readonly persistentAdjustment?: {
    readonly directives?: readonly string[]
  } | null
}
...
const PERSONALITY_DIRECTIONS: Readonly<Record<string, string>> = {
...
function directives(value: unknown): readonly string[] {
  const record = objectRecord(value)
  const raw = record?.directives
  return Array.isArray(raw) ? raw.filter((item): item is string => typeof item === 'string') : []
}

function parseGallery(value: unknown): readonly PersonalityVoiceExample[] {
  if (!Array.isArray(value)) return []
  const examples: PersonalityVoiceExample[] = []
  for (const raw of value) {
    const item = objectRecord(raw)
    const situation = nonEmptyText(item?.situation)
    const line = nonEmptyText(item?.line)
    if (situation && line) examples.push({ situation: situation.trim(), line: line.trim() })
  }
  return examples
}

export function parsePersonalitySnapshot(value: unknown): PersonalitySnapshot {
  const record = objectRecord(value)
...
function rotationOffset(seed: string, total: number): number {
  if (total <= 0) return 0
 ...
 */
export function renderRotatingGallery(
  value: unknown,
  seed: string,
  count = 3
): string | null {
  const { gallery } = parsePersonalitySnapshot(value)
  if (gallery.length === 0 || count <= 0) return null
  const size = Math.min(count, gallery.length)
  const offset = rotationOffset(seed, gallery.length)
  const selected = Array.from(
    { length: size },
    (_, index) => gallery[(offset + index) % gallery.length] as PersonalityVoiceExample
  )
  return [
...
export function renderCompactVoiceExample(value: unknown, seed = ''): string | null {
...
export function personalityId(personality: PersonalitySnapshot): string {
  return personality.manifest?.id ?? 'personalidade-canônica-indisponível'
}

function defaultContinuityAnchor(personality: PersonalitySnapshot): string {
  return [
...
export function renderFullPersonalityInstruction(value: unknown): string {
...
export function renderCompactPersonalityInstruction(value: unknown): string {
  const personality = parsePersonalitySnapshot(value)
  return [
...
export function renderCompactPersonalityAnchor(value: unknown, degradation: string | null = null): string {
  const personality = parsePersonalitySnapshot(value)
  return [
...
function mapDirections(values: readonly string[]): string[] {
  return values
    .map((id) => PERSONALITY_DIRECTIONS[id])
    .filter((item): item is string => typeof item === 'string')
}

export function renderTurnPersonalityAdjustment(value: unknown): string | null {
...
export function renderPersistentPersonalityDirection(value: unknown): string | null {
  const feedback = objectRecord(value)
