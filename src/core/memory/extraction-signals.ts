...
export function isTransientMemory(text: string): boolean {
...
export function declaredProjectFact(value: string): { project: string; text: string } | null {
  const text = ownerStatement(value).trim()
  if (!text || /\?|\b(?:talvez|se fosse|poderia|acho que)\b/iu.test(text)) return null
...
export function hasUnattributedOwnerContent(text: string): boolean {
