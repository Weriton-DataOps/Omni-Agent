...
test('manifesto e núcleo formam uma identidade única', async () => {
  ...
  assert.match(persona.nucleus, /Pedido já feito não volta como lição de casa/i)
  assert.match(persona.nucleus, /pertencem ao Omni, não ao proprietário/i)
  assert.match(persona.textAdapter, /assuma-a silenciosamente/i)
  assert.doesNotMatch(persona.nucleus, /Roda e me mostra|até você abrir a caixa e rodar/i)
