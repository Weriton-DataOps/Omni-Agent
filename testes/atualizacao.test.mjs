...
function runner({ before = '0.12.0', after = '0.13.0', marketplace = canonicalMarketplace } = {}) {
  const calls = []
...
test('atualiza, valida e orienta a aplicação conforme a interface', async () => {
  ...
  const result = await atualizarPlugin({
    ...
    checkVersion: async () => ({ latestVersion: '0.13.0', status: 'outdated' })
  })

  assert.equal(result.status, 'updated')
  assert.equal(result.previousInstalledVersion, '0.12.0')
  assert.equal(result.installedVersion, '0.13.0')
...
test('versão atual é validada sem pedir nova sessão', async () => {
  const fake = runner({ before: '0.12.0', after: '0.12.0' })
  const result = await atualizarPlugin({
    casa: 'C:\\omni-test',
    run: fake.run,
    resolveCli: async () => 'claude-test',
    checkVersion: async () => ({ latestVersion: '0.12.0', status: 'current' })
  })
