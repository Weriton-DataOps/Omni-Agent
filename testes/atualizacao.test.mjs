...
function runner({ before = '0.9.1', after = '0.10.0', marketplace = canonicalMarketplace } = {}) {
  const calls = []
...
test('atualiza, valida e orienta a aplicação conforme a interface', async () => {
  ...
  assert.equal(result.previousInstalledVersion, '0.9.1')
...
test('versão atual é validada sem pedir nova sessão', async () => {
  const fake = runner({ before: '0.9.1', after: '0.9.1' })
  const result = await atualizarPlugin({
    casa: 'C:\\omni-test',
    run: fake.run,
    resolveCli: async () => 'claude-test',
    checkVersion: async () => ({ latestVersion: '0.9.1', status: 'current' })
  })
