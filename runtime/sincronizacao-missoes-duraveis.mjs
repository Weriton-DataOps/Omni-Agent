...
export async function sincronizarMissoesDuraveis(casa) {
  ...
  for (const session of cycle.sessions) {
    ...
    await broker.upsertMission({ id: `mission-${session.id.replace(/^session-/, '')}`, objective: session.objective, state: state(session.state), priority: 70, payload: { sessionId: session.id, currentStep: session.currentStep, openTasks: session.openTasks, nextAction: session.nextAction ?? null, dueAt: session.dueAt ?? null, scheduledAt: session.scheduledAt ?? null, lastMovementAt: session.lastMovementAt ?? session.updatedAt }, createdAt: session.startedAt, updatedAt: session.updatedAt, closedAt: session.state === 'closed' ? session.updatedAt : null })
    count += 1
  }
