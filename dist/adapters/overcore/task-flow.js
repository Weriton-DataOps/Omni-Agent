...
function safeDocument(value) {
    ...
    if (encoded.length > 250_000)
        throw new Error('Documento de tarefa excede o limite do cliente.');
...
function changes(value) {
    ...
    for (const key of Object.keys(result))
        if (!editable.has(key) || metadata.has(key))
            throw new Error(`Campo não editável da revisão: ${key}.`);
    return result;
}
/** Only the protocol adapter knows this product. No task engine is imported. */
export class OvercoreHttpClient {
    token;
    transport;
    identity;
    constructor(endpoint, token, transport = fetch) {
        this.token = token;
        ...
        if (token.length < 16 || /[\r\n]/u.test(token))
            throw new Error('Token local do Overcore ausente ou inválido.');
        this.identity = url.origin;
    }
    async request(path, method, body) {
        if (!/^\/v1\/(?:preflight(?:\/[A-Za-z0-9._:-]+\/admit)?|tasks\/[A-Za-z0-9._:-]+(?:\/(?:cancel|resume))?)$/u.test(path))
        ...
        try {
            ...
            throw new Error('Overcore indisponível ou tempo esgotado. Retome o mesmo fluxo; não crie outro pedido.');
        }
        if (!response.ok)
            throw new Error(`Overcore respondeu HTTP ${response.status}. O vínculo foi preservado; consulte o mesmo fluxo para confirmar o estado remoto.`);
        const raw = await response.text();
        if (raw.length > 2_000_000)
            throw new Error('Resposta externa excedeu o limite.');
        try {
            return safeDocument(JSON.parse(raw));
        }
        catch {
            throw new Error('Overcore devolveu documento inválido; o vínculo foi preservado.');
        }
    }
}
export async function taskFlowClient(env = process.env) {
    ...
    if (!token && env.LOCALAPPDATA) {
        try {
            ...
            if (configuration.schemaVersion === 1 && typeof configuration.localToken === 'string')
                token = configuration.localToken;
        }
        catch { /* The caller receives a configuration error, never the file or secret. */ }
    }
    if (!token)
        throw new Error('Integração ainda sem token local no ambiente ou na configuração privada do Omni.');
    let endpoint = env.OVERCORE_URL;
    if (!endpoint) {
        if (!env.LOCALAPPDATA)
            throw new Error('Descritor do Overcore indisponível.');
        const descriptor = object(JSON.parse(await readFile(join(env.LOCALAPPDATA, 'Overcore', 'runtime.json'), 'utf8')), 'Descritor');
        if (descriptor.host !== '127.0.0.1' || !Number.isInteger(descriptor.port) || Number(descriptor.port) < 1 || Number(descriptor.port) > 65535)
            throw new Error('Descritor local do Overcore inválido.');
        endpoint = `http://127.0.0.1:${descriptor.port}`;
    }
    return new OvercoreHttpClient(endpoint, token);
}
export async function taskFlowConfigured(env = process.env) {
    try {
        await taskFlowClient(env);
        return true;
    }
    catch {
        return false;
...
export class OvercoreTaskFlow {
    api;
    directory;
    store = new NodeLocalJsonStore();
    // Lock only this flow, never memory or other conversations. Heartbeat survives network waits.
    lock = new NodeLocalFileLock({ acquisitionTimeoutMs: 2000 });
    constructor(home, api) {
        this.api = api;
        if (!isAbsolute(home))
            throw new Error('Casa do Omni deve ser absoluta.');
        this.directory = join(home, 'runtime', 'external-task-flows');
    }
    path(flowId) {
        if (!/^flow-[a-f0-9]{24}$/u.test(flowId))
            throw new Error('Identidade do fluxo inválida.');
        return join(this.directory, `${flowId}.json`);
    }
    async read(sessionId, flowId) {
        const flow = object(await this.store.read(this.path(flowId)), 'Fluxo');
        if (flow.schemaVersion !== 1 || flow.flowId !== flowId || flow.sessionKey !== fingerprints.fingerprint(sessionId).value)
            throw new Error('Fluxo ausente, incompatível ou pertencente a outra conversa.');
        return flow;
    }
    async save(flow) { await this.store.write(this.path(flow.flowId), flow); }
    async start(sessionId, idempotencyKey, input) {
        text(sessionId, 'Conversa');
        text(idempotencyKey, 'Chave do pedido');
        const fields = changes(input);
        for (const key of ['objective', 'context', 'discoveryAuthority', 'availableExecutionAuthority']) {
            if (fields[key] === undefined)
                throw new Error(`Pedido sem ${key}; o Omni deve estruturar o que já foi autorizado.`);
        }
        ...
        return this.lock.run(`${this.path(flowId)}.operation.lock`, async () => {
            ...
            if (await this.store.read(this.path(flowId))) {
                ...
                if (existing.inputDigest !== digest)
                    throw new Error('A mesma chave do pedido recebeu outro conteúdo. Use a resposta do fluxo existente ou uma nova intenção.');
    ...
    async answer(sessionId, flowId, input) {
        ...
        if (!Object.keys(patch).length)
            throw new Error('Materialize a decisão nos campos do pedido; IDs sozinhos não resolvem a lacuna.');
        ...
        return this.lock.run(`${this.path(flowId)}.operation.lock`, async () => {
            ...
            if (flow.answerReportId === reportId) {
                if (flow.answerDigest !== digest)
                    throw new Error('Este relatório já foi respondido com outro conteúdo.');
                return this.advance(flow);
            }
            const report = flow.report;
            if (flow.taskId || !report || report.status !== 'decisions-required' || report.reportId !== reportId)
                throw new Error('Resposta não corresponde ao pacote pendente desta conversa.');
            const decisions = objects(report.requiredDecisions, 'Decisões');
            if (selected.length !== decisions.length || new Set(selected.map(a => a.decisionId)).size !== selected.length)
                throw new Error('Responda cada decisão do pacote uma única vez.');
            const at = new Date().toISOString();
            const answers = selected.map(selection => {
                const decision = decisions.find(d => d.decisionId === selection.decisionId);
                if (!decision || !objects(decision.options, 'Opções').some(option => option.optionId === selection.optionId))
                    throw new Error('Decisão/opção não pertence ao relatório pendente.');
                return {
                    ...
                    sourceReport: { reportId, draftRevision: report.draftRevision, draftFingerprint: report.draftFingerprint, reportFingerprint: fingerprints.fingerprint(report) }
                };
            });
    ...
    async advance(flow) {
        if (!flow.report) {
            ...
            if (report.draftId !== flow.draft.draftId || report.draftRevision !== flow.draft.revision ||
                ...
                !/^[A-Za-z0-9._:-]+$/u.test(text(report.reportId, 'Report ID')))
                throw new Error('Relatório devolvido não corresponde à revisão enviada.');
            objects(report.requiredDecisions, 'Decisões');
            if (report.status === 'ready' && report.requiredDecisions.length)
                throw new Error('Relatório ready contradiz decisões pendentes.');
            flow.report = report;
        ...
        if (flow.report.status === 'ready' && !flow.taskId) {
            ...
            if (receipt.reportId !== flow.report.reportId || !/^[A-Za-z0-9._:-]+$/u.test(text(receipt.taskId, 'Task ID')))
                throw new Error('Recibo não corresponde ao relatório admitido.');
        ...
        if (flow.taskId) {
            ...
            if (task.taskId !== flow.taskId || request.idempotencyKey !== flow.draft.executionIdempotencyKey ||
                ...
                request.correlationId !== flow.draft.correlationId)
                throw new Error('Estado devolvido não pertence ao pedido da conversa.');
            if (!['accepted', 'planning', 'ready', 'running', 'verifying', 'blocked', 'cancelling', 'cancelled', 'succeeded', 'failed'].includes(String(task.status)))
                throw new Error('Estado remoto desconhecido.');
            if (terminal.has(String(task.status)) || task.status === 'blocked') {
                ...
                if (result.taskId !== flow.taskId || result.status !== task.status || result.requestId !== request.requestId ||
                    canonicalJson(result.requestFingerprint) !== canonicalJson(fingerprints.fingerprint(request)))
                    throw new Error('Resultado não corresponde à tarefa/estado consultados.');
                text(result.resultId, 'Resultado ID');
                text(result.summary, 'Resumo do resultado');
                objects(result.criteria, 'Critérios');
                objects(result.evidence, 'Evidências');
            }
...
export async function listTaskFlows(home, sessionId) {
    ...
    for (const name of names.filter(name => /^flow-[a-f0-9]{24}\.json$/u.test(name))) {
        const flow = object(JSON.parse(await readFile(join(directory, name), 'utf8')), 'Fluxo');
        if (flow.schemaVersion === 1 && flow.sessionKey === key)
            flows.push(flowView(flow));
    }
    return flows;
}
