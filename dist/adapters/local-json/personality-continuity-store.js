...
function parse(value) {
    ...
    const observations = item.observations.map((raw) => {
        ...
        const row = raw;
        if (Object.keys(row).length !== 2 || typeof row.fingerprint !== 'string' || !/^[a-f0-9]{64}$/u.test(row.fingerprint) || typeof row.recordedAt !== 'string' || !Number.isFinite(Date.parse(row.recordedAt)))
            throw new Error('Invalid continuity observation.');
...
export async function observePersonalityContinuity(home, feedback, at) {
    if (!hasContinuityComplaint(feedback))
        return;
