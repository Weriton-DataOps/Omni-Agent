...
const KEYS = ['credentialId', 'version', 'revision', 'providerRef', 'accountRef', 'environmentRef', 'secretRef', 'issuedAt', 'expiryKind', 'expirySource', 'expiresAt', 'status', 'statusChangedAt', 'lastCheckedAt', 'lastSuccessAt', 'unusableSince', 'lastFailureAt', 'failureCode', 'evidenceRef', 'revokedAt', 'replacedById', 'renewalMode', 'renewBeforeSeconds', 'nextCheckAt', 'nextRetryAt'];
...
function timestamp(value, path) {
    ...
    if (!Number.isFinite(Date.parse(result)) || new Date(result).toISOString() !== result)
        throw new Error(`${path}: invalid UTC timestamp`);
    return result;
}
function nullableTime(value, path) {
    return value === null ? null : timestamp(value, path);
}
export function parseCredentialMetadata(value) {
    ...
    if ((parsed.expiryKind === 'known') !== (parsed.expiresAt !== null))
        throw new Error('expiryKind and expiresAt disagree');
    if (parsed.expiryKind !== 'unknown' && parsed.expirySource === 'unknown')
        throw new Error('Explicit expiry requires provenance');
    if (parsed.issuedAt !== null && parsed.expiresAt !== null && parsed.issuedAt >= parsed.expiresAt)
        throw new Error('Expiration must follow issuance');
    if (parsed.status === 'active' && parsed.lastSuccessAt === null)
        throw new Error('Active credential requires verification');
    if (parsed.status === 'revoked' && parsed.revokedAt === null)
        throw new Error('Revocation requires a timestamp');
    if (parsed.status === 'replaced' && (parsed.replacedById === null || parsed.replacedById === parsed.credentialId))
        throw new Error('Replacement requires a new identity');
    return parsed;
}
