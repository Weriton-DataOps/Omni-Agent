...
export function ownerStatement(value) {
    return value
        .replace(/```[\s\S]*?```/gu, '')
        .replace(/^\s*>.*$/gmu, '')
        .replace(/[“"]([^”"\n]*)[”"]/gu, (_match, content) => /^(?:[A-Za-z]:[\\/]|\/|\\\\)\S+$/u.test(content) ? content : '')
        .replace(/\b(?:o cliente|a cliente|ele|ela|o documento|o teste)\s+(?:disse|diz|escreveu|pediu|afirma)(?:\s+que)?[^.!?;\n]*/giu, '');
}
export function normalizeOwnerText(value) {
    return ownerStatement(value).normalize('NFD').replace(/[\u0300-\u036f]/gu, '').toLowerCase().replace(/\s+/gu, ' ').trim();
}
export function hasContinuityComplaint(value) {
    const text = normalizeOwnerText(value);
    if (/^(?:explique|defina|compare|no teste|no exemplo|use|escreva|nao diga)\b/u.test(text))
        return false;
...
export function hasExplicitPersistentPersonalityDirective(value) {
    const text = normalizeOwnerText(value);
    if (/^(?:explique|defina|compare|no teste|no exemplo|use|escreva|nao diga)\b/u.test(text))
        return false;
