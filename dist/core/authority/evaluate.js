function grantCovers(ceiling, action) {
...
export function evaluateAuthority(envelope, now) {
    const boundaryTriggered = envelope.triggeredBoundaries.length > 0;
    const actionDecisions = envelope.actions.map((action) => {
        const reasonCode = boundaryTriggered
            ? 'risk-policy'
